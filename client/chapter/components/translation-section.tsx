      ...
      {chapter.verses.map((verse) => (
        <div id={`verse_${verse.id}`} className="bg-background p-7 rounded-md shadow-md" key={verse.id}>
          <div className="flex items-center justify-between gap-7 mb-6">
            ...
            <h2 className="text-2xl font-bold text-justify" style={{ fontFamily: "uthmanic", textAlignLast: "right" }}>
              {verse.text}
            </h2>
          </div>
          <div>
            ...
            <p className="py-3 text-lg text-foreground/80">{verse.translation}</p>
          </div>
          <Separator />
